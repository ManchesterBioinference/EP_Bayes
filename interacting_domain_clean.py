def interacting_domains(chrom_pro_coord, chrom_ER_coord, chrom, state, matrix_version = True, chromosome_version = False): # state 'left' or 'right'		
	import numpy as np

	selective = False # cares about whether a domain exists or not
	import config_variables
	data_folder = config_variables.data_folder
	if chromosome_version:
		interacting_domains = np.loadtxt("hg_19_sizes.csv", dtype = str, delimiter = "\t")
		interacting_domains_chrom = interacting_domains[interacting_domains[:,0] == chrom][:,1][0].astype(int)
		size_domains = 3
		bins = np.array([-1.*interacting_domains_chrom, 0., interacting_domains_chrom, 2.*interacting_domains_chrom])
		existing_domains = np.ones(bins.size, bool)
