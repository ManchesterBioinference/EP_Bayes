def execute(sensitivity_match_MAP, number_of_interacting_enhancers_, option_to_plot = "ALL", type_of_models = [], posterior_MOG = None, kappa_0=1.0, mu_0=1.0, alpha_0=1.0, Beta_0=1.0, number_of_samples=10, burn_in=0):
	...
	if "MOG_correl_dist" in type_of_models or "MOG_dist" in type_of_models: name_of_output_FDR_file += "_MOG_{0}_{1}_{2}_{3}_{4}_{5}".format(kappa_0, mu_0, alpha_0, Beta_0, "_".join(np.array(number_of_samples, str)), "_".join(np.array(burn_in, str)))
	...
	for domain_atr, domain, invert_domain, thresh, give_indexes_for_thresholds in np.array([[None, False, False, True, False], ["within_domain", True, False, False, True], ["outside_domain", True, True, False, True]]):
		...
		pink_line = mlines.Line2D([], [], color='cyan', marker='*', markersize = red_blue_yellow_cyan_marker_size_legend_box, label='LVM data+prior')
		green_line = mlines.Line2D([], [], color='green', marker="v", markersize = red_blue_yellow_cyan_marker_size_legend_box, label='LVM prior')
		...
		for index_filt_val, filter_value in enumerate(filter_values_):
			...
			for index_opt, option_ in zip(range(len(selected_combinations)), selected_combinations):#[1:3]
				...
				if domain_atr <> None:
					...
					sensitivity_dist_MOG = sensitivity_match_MAP["MOG_dist"][domain_atr]
					sensitivity_dist_correl_MOG = sensitivity_match_MAP["MOG_correl_dist"][domain_atr][comb_MOG]
					...
					sensitivity_dist_MOG = sensitivity_match_MAP["MOG_dist"]["unsplit"]
					sensitivity_dist_correl_MOG = sensitivity_match_MAP["MOG_correl_dist"]["unsplit"][comb_MOG]
				...
				if "MOG_correl_dist" in type_of_models: 
					if not(domain_atr): posterior_correl_dist_true_unsplit_MOG, posterior_correl_dist_false_unsplit_MOG = posterior_MOG["positive_interactions"]["MOG_correl_dist"][comb_MOG], posterior_MOG["negative_interactions"]["MOG_correl_dist"][comb_MOG]
				...
				if "MOG_correl_dist" in type_of_models: 
					...
					ax[index_opt].plot(ind, probabilities_dist_correl_MOG, alpha=1.0, color="cyan", marker= "*", linewidth=0.0, markersize=red_blue_yellow_cyan_marker_size, label = "LVM data+prior")
				...
				if "MOG_dist" in type_of_models:
					...
					ax[index_opt].plot(ind, probabilities_dist_MOG, alpha=1.0, color="green", marker= "v", linewidth=0.0, markersize=red_blue_yellow_cyan_marker_size, label = "LVM prior")
