...
# Iterator function
	def sample(self,iters):
		...
		for k in range(iters):
			...
			self.update_c_new()
			print self.c
			#self.total_dist += self.dist[np.arange(self.J), self.c[self.K:]].tolist()
...
#		return values[np.digitize(random_sample(size), bins)]
	...
	def update_c_new(self): # c_i = j| c_-i,alpha) 
		...
		if self.mode_of_sampler == "dirichlet_MOG":
			...
			#for j in range(self.J): self.c[self.K+j] = np.random.choice(self.K, 1, replace=True, p=self.prob[j])
			

			#self.c[self.K:] = self.labels[np.digitize(samples, bins)]
...
def executor(arr):
	...
	mode_of_sampler, number_of_samples, option_correl, chrom = arr
